...
Récupérez ou le tar.gz "Binary Distribution".
...
Ajoutez votre projet dans l'onglet "Deployments". Vous pouvez également modifier votre "Application Context" en le positionnant à "/"

[NOTE]
====
Le "Context" de tomcat, est le chemin de base auquel Tomcat va affecter les requêtes de votre Servlet.
Avec un context qui vaut "/handcraft", vos requêtes devront être envoyées à l'URL "/handcraft/hello" par exemple.
Comme nous souhaitons avoir des URL de forme "/hello", nous positionnons un context à "/".
====
...
Ecrire un controller qui expose une route "/pokemons".
...
* link:http://localhost:8080/pokemons?id=25[,window="_blank"]
* link:http://localhost:8080/pokemons?id=145[,window="_blank"]
* link:http://localhost:8080/pokemons?name=pikachu[,window="_blank"]
* link:http://localhost:8080/pokemons?name=zapdos[,window="_blank"]